...
func makeWriter(batchSize int, interval time.Duration) (*Writer, *[]aggregation.MeasurementRecord) {
	flushed := &[]aggregation.MeasurementRecord{}
	mu := &sync.Mutex{}
...
func TestWriteBuffers(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		_ = w.Write(ctx, aggregation.MeasurementRecord{Node: "n"})
	}
	w.mu.Lock()
	got := len(w.buf)
	w.mu.Unlock()
...
func TestFlushDrainsBuffer(t *testing.T) {
	...
	flushed := make([]aggregation.MeasurementRecord, 0)
	var mu sync.Mutex
	...
	w := &Writer{
		...
		batchSize:     10,
		buf:           make([]aggregation.MeasurementRecord, 0, 10),
		flushCh:       make(chan struct{}, 1),
		done:          make(chan struct{}),
	}

	// Monkey-patch writeBatch via closure — capture into flushed slice.
	origFlush := func(ctx context.Context, records []aggregation.MeasurementRecord) error {
		mu.Lock()
	...
	for i := 0; i < 4; i++ {
		w.buf = append(w.buf, aggregation.MeasurementRecord{Node: "n", OutputTokens: uint64(i)})
	}
