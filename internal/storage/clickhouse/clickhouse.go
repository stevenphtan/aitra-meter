// Package clickhouse provides a buffered async ClickHouse storage Backend.
//
// The package registers itself in init() so the aggregation service can
// select it by name:
//
//	import _ "github.com/aitra-ai/aitra-meter/internal/storage/clickhouse"
//	b, err := storage.New("clickhouse", map[string]string{"dsn": "..."})
package clickhouse
...
func init() {
	storage.Register("clickhouse", func(config map[string]string) (storage.Backend, error) {
		dsn := config["dsn"]
		if dsn == "" {
			return nil, fmt.Errorf("clickhouse: dsn is required")
		}
		return New(context.Background(), Config{DSN: dsn}, zap.NewNop())
	})
}
...
func (w *Writer) Name() string { return "clickhouse" }
...
func (w *Writer) Write(_ context.Context, r model.MeasurementRecord) error {
	w.mu.Lock()
...
// WriteBatch buffers a slice of records. Each record is enqueued individually;
// a flush is triggered if the batch size is reached.
func (w *Writer) WriteBatch(ctx context.Context, rs []model.MeasurementRecord) error {
	for _, r := range rs {
...
// EnergyJoulesPUE is computed server-side by multiplying raw joules by q.PUE.
func (w *Writer) QueryChargeback(ctx context.Context, q storage.ChargebackQuery) ([]storage.NamespaceCharge, error) {
	rows, err := w.conn.Query(ctx, `
		SELECT
			namespace,
			sum(energy_joules)       AS energy_raw,
			sum(output_tokens)       AS tokens,
			any(attribution_method)  AS method,
			any(team)                AS team,
			any(cost_centre)         AS cc
		...
		ch.Named("cluster", q.Cluster),
		ch.Named("from", q.From),
		ch.Named("to", q.To),
	)
	if err != nil {
		return nil, fmt.Errorf("clickhouse chargeback query: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			return nil, fmt.Errorf("clickhouse chargeback scan: %w", err)
		}
...
func (w *Writer) writeBatch(ctx context.Context, records []model.MeasurementRecord) error {
